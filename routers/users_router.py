...
async def update_user(
    ...
    logger.info(f'Пользователь {user.username} обновил свои данные на : [{new_username, new_email, new_password}]')
...
async def update_role_user(
    user_id: int, new_role: str, user_admin: User = Depends(get_admin_user)
):
    user: User = await UsersDAO.find_one_or_none(id=user_id)
    if not user:
        raise UserNotFound
    logger.info(f'Администратор {user_admin.username} изменил роль пользователю {user.username}')
...
async def ban_user(username: str, user_admin: User = Depends(get_admin_user)):
    ...
    if await check_user(user, user_admin):
        if not user.is_banned:
            logger.info(f'Администратор {user_admin.username} заблокировал пользователя: {user.username}')
...
async def unban_user(
    ...
    if await check_user(user, user_admin):
        if user.is_banned:
            logger.info(f'Администратор {user_admin.username} разблокировал пользователя: {user.username}')
